...
class FsmCore {
  ...
  configure(config) {
    ...
    this.registerInitialState(config.initialState);
    this.registerInitialStateData(config.initialStateData);

    config.transitions.forEach((transition) => {
      this.registerTransition(transition);
    });

    config.afterTransitions.forEach((transition) => {
   ...
   */
  generateTransition(state, stateData) {
